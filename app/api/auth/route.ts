...
function usernameFrom(email: string) {
  ...
  return username.length >= 3 ? username : `user_${Date.now()}`;
}
async function updateAuthenticatedUser(token: string, body: Row) {
  ...
  const payload: Row = text
    ...
    : {};
  if (!response.ok)
    throw new Error(String(payload.message || payload.msg || "密码更新失败"));
  return payload;
}
  ...
  try {
    ...
    if (action === "refresh") {
      ...
      return Response.json(
        await authRequest<Session>("token?grant_type=refresh_token", {
    ...
    if (action === "reset-password") {
      ...
      const session = await authRequest<Session>("verify", {
        email,
        token: code,
        type: "recovery",
      });
    ...
    if (action === "change-password") {
      ...
      if (!user.email) throw new Error("当前账户缺少邮箱，无法验证密码");
      await authRequest<Session>("token?grant_type=password", {
        email: user.email,
        password: currentPassword,
      });
      await updateAuthenticatedUser(request.headers.get("authorization")!.slice(7), {
        password,
    ...
    if (action === "update-display-name") {
      ...
      await dbRequest(`profiles?id=eq.${user.id}`, {
        method: "PATCH",
    ...
    if (action === "admin-create-user") {
      ...
      if (!validEmail(email)) throw new Error("请输入有效邮箱");
      validateNewPassword(password, confirmPassword);
      const profileExists = await dbRequest<Row[]>(
        `profiles?select=id&email=eq.${encodeURIComponent(email)}&limit=1`,
      );
      if (profileExists.length) throw new Error("该邮箱已经开通账户");
      const displayName = String(body.displayName || email.split("@")[0]).trim().slice(0, 50) || "学习者";
      const created = await authRequest<{ id?: string; user?: { id?: string } }>(
        "admin/users",
        {
          email,
          password,
    ...
    if (!validEmail(email) || password.length < 8)
      return Response.json({ error: "请输入有效邮箱，密码至少 8 位" }, { status: 400 });

    if (action === "activate") {
      validateNewPassword(password, confirmPassword);
      const profiles = await dbRequest<Row[]>("profiles?select=id&limit=1");
      if (profiles.length)
        return Response.json({ error: "首次管理员已经建立。新账户只能由管理员创建。" }, { status: 403 });
      await authRequest(
        "admin/users",
        {
          email,
          password,
          email_confirm: true,
          user_metadata: { display_name: String(body.displayName || "Echo").trim() || "Echo" },
          app_metadata: { role: "admin" },
        },
        true,
      );
    } else if (action !== "login") {
      return Response.json({ error: "未知登录操作" }, { status: 400 });
    }
    return Response.json(
      await authRequest<Session>("token?grant_type=password", { email, password }),
    );
  } catch (error) {
    return Response.json(
